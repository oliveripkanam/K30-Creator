...
import { Info, ChevronRight, TrendingUp, Clock, Target, Star, Coins, RefreshCw, AlertCircle } from 'lucide-react';
...
// Mock data
const mockDecodes: DecodeAttempt[] = [
  {
    id: '1',
    date: '15 Sep, 14:30',
    marks: 8,
...
const mockAggregates: PerformanceAggregates = {
  avgAccuracy: 78,
  attemptCount: 8,
  avgMarks: 5.4,
  medianTime: 9,
...
const SparklineChart: React.FC<{
  ...
  // Generate X-axis tick positions (4-6 evenly spaced)
  const xTicks = data.length > 1 ? 
    Array.from({ length: Math.min(6, data.length) }, (_, i) => {
      ...
      return {
        ...
        label: data[dataIndex]?.date.split(',')[0] || '' // Just the date part
      };
    }) : [];
    ...
    <Card className={`w-full ${compact ? 'p-4' : 'p-6'} space-y-4`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        <KPIChip
          ...
          label="Avg Accuracy"
          value={`${aggregates.avgAccuracy}%`}
          valueColor={aggregates.avgAccuracy >= 70 ? 'text-green-600' : 'text-gray-900'}
        />
        <KPIChip
          icon={<TrendingUp className="w-4 h-4" />}
          label="Attempts"
          value={aggregates.attemptCount.toString()}
        />
        <KPIChip
          icon={<Star className="w-4 h-4" />}
          label="Avg Marks/Q"
          value={aggregates.avgMarks.toString()}
        />
        <KPIChip
          icon={<Clock className="w-4 h-4" />}
          label="Median Time"
          value={`${aggregates.medianTime} min`}
        />
        <KPIChip
          icon={<Coins className="w-4 h-4 text-yellow-600" />}
          label="Tokens Earned"
          value={aggregates.tokensTotal.toString()}
        />
      </div>
        ...
        <div className="space-y-2">
          {decodes.slice(0, 5).map((decode) => (
            <RecentListItem
              key={decode.id}
              decode={decode}
              onClick={() => console.log('View details for:', decode.id)}
            />
          ))}
        </div>
