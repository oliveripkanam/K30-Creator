// Plain TS reimplementation to avoid any build/E SM issues
...
type MCQ = {
  ...
  explanation: string;
  step: number;
  calculationStep?: { formula?: string; substitution?: string; result?: string };
};
...
export default async (req: Request, _context: Context) => {
  try { console.log('[fn ai-decode(ts)] invocation', { method: req.method, url: req.url }); } catch {}
  ...
  const buildUrl = (endpointValue: string, deploymentName: string, version: string): string => {
    ...
    try {
      if (isFull) {
        ...
        if (!/\/chat\/completions(\?|$)/.test(full)) full = `${full}/chat/completions`;
  ...
  let url: string;
  try { url = buildUrl(rawEndpoint, deployment, apiVersion); } catch (e: any) { return respond(500, { error: e?.message || 'Invalid Azure config' }); }
  try { console.log('[fn ai-decode(ts)] config', { endpoint: rawEndpoint, deployment, apiVersion, url }); } catch {}
  ...
  try {
    ...
    try { parsed = JSON.parse(content); } catch { const match = content.match(/\{[\s\S]*\}/); if (match) parsed = JSON.parse(match[0]); }
