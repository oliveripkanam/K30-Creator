  ...
  const handleSubmit = async () => {
    ...
    switch (activeTab) {
      case 'photo':
        ...
        type = 'photo';
        fileData = { base64: await readAsBase64(photoFile), mimeType: photoFile.type || 'image/*', name: photoFile.name };
        break;
      case 'file':
        if (!uploadFile) return;
        content = `File uploaded: ${uploadFile.name}`;
        type = 'file';
        fileData = { base64: await readAsBase64(uploadFile), mimeType: uploadFile.type || 'application/octet-stream', name: uploadFile.name };
        break;
      case 'text':
    ...
    const question: Question = { id: Date.now().toString(), content, marks: decideMarks(), type, timestamp: new Date(), fileData, subject, syllabus, level };
    onSubmit(question);
  };
      ...
      <div className="max-w-6xl mx-auto p-3 sm:p-4 lg:p-6">
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription className="text-sm">Choose how you'd like to input your question.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 sm:space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                ...
                <Select value={subject} onValueChange={setSubject}>
                  <SelectTrigger id="subject" className="mt-2"><SelectValue placeholder="Select subject" /></SelectTrigger>
              ...
              <div>
                ...
                <Select value={syllabus} onValueChange={setSyllabus}>
                  <SelectTrigger id="syllabus" className="mt-2"><SelectValue placeholder="Select syllabus (optional)" /></SelectTrigger>
                  <SelectContent>{SYLLABUS_OPTIONS.map((s) => (<SelectItem key={s} value={s}>{s}</SelectItem>))}</SelectContent>
                </Select>
              </div>
              <div>
                <Label htmlFor="level">Year / Level</Label>
                <Select value={level} onValueChange={setLevel}>
                  <SelectTrigger id="level" className="mt-2"><SelectValue placeholder="Select year/form (optional)" /></SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel className="font-semibold">Year:</SelectLabel>
                      {LEVEL_OPTIONS.filter((s) => s.startsWith('Year ')).map((s) => (<SelectItem key={s} value={s}>{s}</SelectItem>))}
                    </SelectGroup>
                    <SelectSeparator />
                    <SelectGroup>
                      <SelectLabel className="font-semibold">Form:</SelectLabel>
                      {LEVEL_OPTIONS.filter((s) => s.startsWith('Form ')).map((s) => (<SelectItem key={s} value={s}>{s}</SelectItem>))}
                    </SelectGroup>
                  </SelectContent>
                </Select>
              </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid w-full grid-cols-3 h-auto">
                <TabsTrigger value="text" className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2 py-3 sm:py-2"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg><span className="text-xs sm:text-sm">Type Text</span></TabsTrigger>
                <TabsTrigger value="photo" className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2 py-3 sm:py-2"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg><span className="text-xs sm:text-sm">Photo</span></TabsTrigger>
                <TabsTrigger value="file" className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-2 py-3 sm:py-2"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg><span className="text-xs sm:text-sm">File</span></TabsTrigger>
              </TabsList>

              <TabsContent value="text" className="space-y-4">
                <div>
                  <Label htmlFor="question-text">Question Text</Label>
                  <Textarea id="question-text" placeholder="Enter your question here... \n\nExample (Mechanics): A ball is thrown horizontally from the top of a building 20m high with an initial velocity of 15 m/s. Calculate the time taken for the ball to reach the ground and the horizontal distance traveled." value={textContent} onChange={(e) => setTextContent(e.target.value)} className="min-h-32 mt-2" />
                  {multiText && (<p className="text-xs text-red-600 mt-2">It looks like there are multiple questions in this text. Please submit one question at a time or crop/split it.</p>)}
                </div>
              </TabsContent>
              ...
              <TabsContent value="photo" className="space-y-4">
                <div>
                  ...
                  <div className={`mt-2 border-2 border-dashed rounded-lg p-6 text-center transition-colors ${isDraggingPhoto ? 'border-purple-400 bg-purple-50' : 'border-gray-300'}`} onDragOver={onPhotoDragOver} onDragEnter={onPhotoDragOver} onDragLeave={onPhotoDragLeave} onDrop={onPhotoDrop}>
                    {photoFile ? (
                      <div className="space-y-2">
                        <div className="w-16 h-16 mx-auto bg-green-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></div>
                        <p className="text-sm">{photoFile.name}</p>
                        <Button variant="outline" size="sm" onClick={() => setPhotoFile(null)}>Remove</Button>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        <div className="w-16 h-16 mx-auto bg-gray-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /></svg></div>
                        <div>
                        ...
                        <Input id="photo-upload" type="file" accept="image/*" capture="environment" onChange={(e) => setPhotoFile(e.target.files?.[0] || null)} className="hidden" />
                        <Button variant="outline" onClick={() => document.getElementById('photo-upload')?.click()}>Choose Photo</Button>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <TabsContent value="file" className="space-y-4">
                <div>
                  ...
                  <div className={`mt-2 border-2 border-dashed rounded-lg p-6 text-center transition-colors ${isDraggingFile ? 'border-blue-400 bg-blue-50' : 'border-gray-300'}`} onDragOver={onFileDragOver} onDragEnter={onFileDragOver} onDragLeave={onFileDragLeave} onDrop={onFileDrop}>
                    {uploadFile ? (
                      <div className="space-y-2">
                        <div className="w-16 h-16 mx-auto bg-blue-100 rounded-full flex items-center justify-center"><svg className="w-8 h-8 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg></div>
                      ...
                      <div className="space-y-3">
                        ...
                        <Input id="file-upload" type="file" accept=".pdf,.doc,.docx" onChange={(e) => setUploadFile(e.target.files?.[0] || null)} className="hidden" />
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                <Input id="marks" type="number" min="1" max="20" value={marks} onChange={(e) => setMarks(parseInt(e.target.value) || 1)} className="w-24" disabled={autoMarks} />
                <div className="grid grid-cols-4 sm:flex gap-2 sm:gap-1 w-full sm:w-auto">
                  {[1, 2, 3, 4, 5, 6, 7, 8].map((mark) => (
                    <Button key={mark} variant={marks === mark ? "default" : "outline"} size="sm" onClick={() => setMarks(mark)} className="h-10 sm:h-8" disabled={autoMarks}>{mark}</Button>
                  ))}
                </div>
                <div className="flex items-center space-x-2">
                  <Button type="button" variant={autoMarks ? 'default' : 'outline'} size="sm" onClick={() => setAutoMarks(v => !v)} className="h-10 sm:h-8">{autoMarks ? 'AI will decide' : 'Let AI decide'}</Button>
                  <Tooltip>
                    ...
                    <TooltipContent sideOffset={6}>Lets the AI choose a suitable number of MCQs based on the question. Your numeric input and quick-pick buttons are disabled while enabled.</TooltipContent>
                  </Tooltip>
                </div>
              </div>
              <p className="text-xs text-muted-foreground">{autoMarks ? 'AI will pick a suitable number of steps after you submit.' : 'This determines how many step-by-step questions we create.'}</p>
            </div>
            ...
            <div className="flex justify-center sm:justify-end pt-4">
              <Button onClick={handleSubmit} disabled={!isValid()} size="lg" className="px-6 sm:px-8 w-full sm:w-auto">
                {'Decode Question'}
