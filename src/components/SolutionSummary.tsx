...
export function SolutionSummaryComponent({ originalQuestion, solution, onComplete, onBack, onSeeReview }: SolutionSummaryProps) {
	...
	const pitfalls = Array.isArray((solution as any).pitfalls) ? (solution as any).pitfalls : [];
			...
			<div className="max-w-4xl mx-auto p-4 space-y-6">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>Mistakes students often make on this topic</CardDescription>
					</CardHeader>
					<CardContent>
						{pitfalls.length ? (
							<ul className="space-y-2">
								{pitfalls.map((p, i) => (
									<li key={i} className="bg-orange-50 p-3 rounded-lg border border-orange-200 text-sm">
										{p}
									</li>
								))}
							</ul>
						) : (
							<p className="text-sm text-muted-foreground">No pitfalls available.</p>
						)}
					</CardContent>
				</Card>
